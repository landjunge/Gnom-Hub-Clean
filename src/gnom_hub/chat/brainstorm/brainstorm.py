...
def _collect_worker_responses(worker_names):
    from gnom_hub.database.legacy_db import get_chat_history, get_active_project; msgs = get_chat_history(get_active_project(), limit=50)
    out = []
...
def dispatch(q, target=None):
    from gnom_hub.database.legacy_db import get_all_agents; ao = [a for a in get_all_agents() if a.get("status") == "online"]
    if target:
        t_low = target.lower()
