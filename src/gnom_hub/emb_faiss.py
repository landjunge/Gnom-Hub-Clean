...
class FaissEmbeddingHelper:
    def __init__(self, model_name: str, db_path: str):
        ...
        if not os.path.exists("data/soul_embeddings.index"): self._create_index()
        else:
    ...
    def _create_index(self):
        ...
        embs = self.model.encode([f"{f[1]}: {f[2]}" for f in facts]) if facts else []
        self.index = faiss.IndexFlatL2(len(embs[0])) if len(embs) > 0 else faiss.IndexFlatL2(384)
    ...
    def add_fact(self, fact_id: str, key: str, value: str):
        self.index.add(self.model.encode([f"{key}: {value}"]))
    ...
    def search(self, query: str, top_k: int = 8) -> list:
        ...
        _, indices = self.index.search(self.model.encode([query]), top_k)
        res = []
