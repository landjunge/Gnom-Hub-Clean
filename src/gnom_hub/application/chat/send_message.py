...
class SendMessageUseCase:
    """Use Case: Nachricht an einen Agenten senden und verarbeiten."""
    ...
    async def execute(self, agent_id: UUID, content: str, role: str = "user") -> ChatMessage:
        ...
        if not agent:
            raise ValidationError(f"Agent mit ID {agent_id} nicht gefunden")

        user_message = ChatMessage(agent_id=agent_id, role=role, content=content)
        await self.chat_repo.save_message(user_message)

        response_content = await self.llm_orchestrator.ask(agent.id, content, model=agent.model)
