from fastapi import APIRouter; from datetime import datetime; import uuid, re; from .db import get_db, save_db, get_active_project; from .brainstorm import dispatch; from .chat_commands import handle_clear, handle_status, handle_job, handle_free, handle_git, handle_publish; from pydantic import BaseModel
router = APIRouter()
...
def _handle_sys(q, m):
    ...
    return {"status": "ok"}
CMDS = {"clear": handle_clear, "status": lambda q: handle_status(), "job": handle_job, "free": handle_free, "git": handle_git, "project": lambda q: _handle_sys(q,"proj"), "publish": handle_publish}
