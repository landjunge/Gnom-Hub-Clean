...
def _handle_sys(q, m):
    ...
    if m=="prov": from agents.provider_switchAG import set_provider; p=q.split(); _post_chat("System", set_provider(p[0], p[1] if len(p)>1 else None)) if p else None
    elif m=="vis": from agents.visionAG import vision_loop; _post_chat("System", vision_loop(q))
    elif m=="evol": from agents.evolutionAG import evolve_agent; _post_chat("System", evolve_agent(q))
    elif m=="proj": from .db import set_active_project; set_active_project(q or "default"); _post_chat("System", f"Project: {q or 'default'}")
    elif m=="brow": from agents.browserAG import browser_cmd; _post_chat("System", browser_cmd(q))
    else: from agents.desktopAG import desktop_action; _post_chat("System", desktop_action(q))
    return {"status": "ok"}
...
def post_chat(msg: ChatMsg):
    ...
    if not cmd and not tgt: from agents.gatekeeperAG import intercept; return intercept(msg.content)
