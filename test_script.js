    ...
    async function nukeHub() {
      const btn = document.getElementById('nuke-btn');
      toast('☢️ NUKE — Alle Prozesse werden gekillt...', 'error');
      try {
      ...
      const reconnect = setInterval(async () => {
        attempts++;
        try {
          const r = await fetch((API || '/api').replace('/api', '') + '/api/health', { signal: AbortSignal.timeout(1000) });
          if (r.ok) {
            ...
            _nukeArmed = false;
            toast('🟢 Hub ist zurück!', 'success');
            setTimeout(() => { btn.classList.remove('nuke-ready'); location.reload(); }, 1200);
          }
        } catch(e) { /* noch tot */ }
        if (attempts > 30) {
          clearInterval(reconnect);
          toast('⚠️ Hub kommt nicht zurück. Manuell starten: gnom-hub', 'error');
          _nukeArmed = false;
        }
      }, 2000);
    }
  ...
  `;
    ...
    (async () => {
      ...
      await discoverPort();
      if (!API) {
        document.getElementById('content').innerHTML = '<div class="empty" style="padding:80px 20px"><h2 style="color:var(--text);margin-bottom:10px">Hub unreachable</h2><p>Start backend on port 3002.</p></div>';
        return;
      }
      await loadAgents();
      updateLamps(agents);
